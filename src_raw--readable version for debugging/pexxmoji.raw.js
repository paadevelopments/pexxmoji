 ...
 * Note : This library was written and optimized purposely for Pexxa
 * Dependencies : [ "jquery", "pexxmoji.raw.css", "pexxmoji.emo_set.[ a - i ].css" }
 * License : MIT
 *
 */
...
var pexxmoji = function(jquery, win, input, etray, tab_icons, copts){ const self = this;
	if (etray.find('.pm_main').length != 0) return etray.find('.pm_mo_acts:eq(0)').click();
	const default_icons = [ '🕒', '🙂', '🐱', '☕', '🎮', '🚖', '💡', '💓', '🚩' ];
	const default_dopts = {
		focus : function(event){ /* natm */ }, blur : function(event){ /* natm */ },
		input : function(event){ /* natm */ }
	};
	if (input.length == 0) return function(){ console.log('no input found'); return false;
	}.call({});
	if (etray.length == 0) return function(){ console.log('no tray found'); return false;
	}.call({});
	self.input = input;
	self.etray = etray;
	self.runna = false;
...
pexxmoji.prototype.setup_init = function(){ const self = this;
	self.input.attr('onkeypress', 'javascript:return event.which != 13');
	self.etray.html('');
	let a = self.window.document.createElement('div');
	a.setAttribute('class','pm_main');
	self.etray.append(a);
	let b = self.window.document.createElement('div');
	b.setAttribute('class','pm_main_one');
	a.append(b);
	let c = self.window.document.createElement('div');
	c.setAttribute('class','pm_main_two');
	a.append(c);
	for (let lo = 0;lo < 9;lo++) { self.utils_cons(lo); }
	for (let lo = 0;lo < 9;lo++) { self.utils_tabs(lo); }
	self.etray.find('.pm_mo_acts').off();
	self.etray.find('.pm_mo_acts').on('click', function(){ let a = self.$(this);
		a.addClass('pm_pmaa');
		a.siblings('button').removeClass('pm_pmaa');
		self.etray.find('.pm_mt_tabs').each(function(){ let aa = self.$(this);
			if (aa.attr('id') == a.data('pm-tg')) {
				aa.show(); self.etray.find('.pm_main_two').scrollTop(0);
			} else { aa.hide(); }
		});
		self.setup_btns(a.data('pm-tg'));
	});
	self.etray.find('.pm_mo_acts:eq(0)').click();
	self.input.data('pm-position', self.utils_gcur());
	clearInterval(self.runna);
	self.input.off('focus blur input');
	self.input.on('focus', function(ev){ let a = self.$(this);
		self.runna = setInterval(function() {
			if (self.$(a).is(':focus')) {
				let aa = a.data('pm-position');
				let bb = self.utils_gcur();
				if (bb != aa) { a.data('pm-position', bb); }
			} else { clearInterval(self.runna) }
		}, 50);
		self.oopts.focus(ev);
	}).on('blur', function(ev){ let a = self.$(this);
		clearInterval(self.runna);
		self.oopts.blur(ev);
	}).on('input', function(ev){ let a = self.$(this);
		self.input.data('pm-position', self.utils_gcur());
		self.oopts.input(ev);
	});
	/* Responsive tray [ Experimental ] */
	/* self.$(self.window).on('resize', function(){ let a = self.etray;
		if (a.width() > 500) { a.find('.pm').attr('style','font-size:'+ 35 +'px;'); }
		else if (a.width() < 500 && a.width() > 370) { a.find('.pm').attr('style','font-size:'+ 30 +'px;'); }
		else { a.find('.pm').attr('style','font-size:'+ 25 +'px;'); }
	}); */
};
pexxmoji.prototype.setup_btns = function(a){ const self = this;
	if (a == 'pm_mt_a') return self.utils_setm(false, '', a);
	if (self.etray.find('#'+ a).html() == '') return function(){
		switch(a){
			case 'pm_mt_b': self.utils_setm(true, em_sm, a); break;
			case 'pm_mt_c': self.utils_setm(true, em_na, a); break;
			case 'pm_mt_d': self.utils_setm(true, em_fo, a); break;
			case 'pm_mt_e': self.utils_setm(true, em_ac, a); break;
			case 'pm_mt_f': self.utils_setm(true, em_pl, a); break;
			case 'pm_mt_g': self.utils_setm(true, em_ob, a); break;
			case 'pm_mt_h': self.utils_setm(true, em_sy, a); break;
			default: self.utils_setm(true, em_fg, a); break;
		}
	}.call({});
};
pexxmoji.prototype.utils_setm = function(a, b, c){ const self = this;
	let ready = false, font_size  = 0, width = self.etray.width()
	if (width > 500) { font_size = 35; }
	else if (width < 500 && width > 370) { font_size = 30; }
	else { font_size = 25; }
	if (!a) {
		self.etray.find('#'+ c).html('');
		let aa = self.utils_emfc();
		if (aa.length == 0) return self.etray.find('#'+ c)
		.html('<div class="pm_mt_tno">Frequently used pexxmoji(s) will display here</div>');
		let bb = 0;
		for (let lo = aa.length - 1;lo >= 0;lo--) { bb += 1;
			let cc = self.window.document.createElement('button');
			if (bb == 9) { cc.setAttribute('class','pm_mt_each pm_icnd'); bb = 0; }
			else { cc.setAttribute('class','pm_mt_each'); }
			let dd = self.utils_emid(aa[lo], {'{':'','}':'','u':''});
		...
		let bb = 0;
		for (let lo = 0;lo < b.length;lo++) { bb += 1;
			let cc = self.window.document.createElement('button');
			if (bb == 9) { cc.setAttribute('class','pm_mt_each pm_icnd'); bb = 0; }
			else { cc.setAttribute('class','pm_mt_each'); }
			let dd = self.utils_emid(b[lo], {'{':'','}':'','u':''});
			cc.innerHTML = '<i class=\'pm pm-' + dd[0] + '\' data-pm-co=\'' + dd[1] +
	...
	if (ready) return function(){
		self.etray.find('#'+ c +' .pm_mt_each').off();
		self.etray.find('#'+ c +' .pm_mt_each').on('click', function(){
			let a = self.$(this).find('i').data('pm-co');
			let b = self.input.scrollTop();
			if ((self.input.scrollTop() + self.input.innerHeight()) >= self.input[0].scrollHeight)
				{ b = self.input[0].scrollHeight }
			let c = self.input.data('pm-position');
			let d = self.input.text();
			let e = d.substr(0, c) + '' + a + '' + d.substr(c);
			self.input.text(e);
			self.utils_srge((c + a.length ));
			self.input.data('pm-position',(c + a.length ));
			self.input.scrollTop(b);
			if (d == '') {
				let z = self.etray.find('#'+ c).scrollTop();
				if (z > 1) { self.etray.find('#'+ c).scrollTop(z - 1); }
				else { self.etray.find('#'+ c).scrollTop(z + 1); }
			}
			self.input.blur();
			let f = self.$(this).find('i').data('pm-va');
			let g = self.utils_emfc();
			let h = true;
			if (g.length > 0) { if (g.includes(f)) { h = false; } }
			if (h) {
				if (g.length > 0) {
					g.push(f); if (g.length > 40) { g.shift(); }
...
pexxmoji.prototype.utils_tabs = function(a){ const self = this;
...
pexxmoji.prototype.utils_cons = function(a){ const self = this;
	let b = [ 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i' ];
	let c = self.window.document.createElement('button');
	if (a == 8) { c.setAttribute('class','pm_mo_acts pm_icnd'); }
	else { c.setAttribute('class','pm_mo_acts'); }
	c.setAttribute('data-pm-tg', 'pm_mt_'+ b[a]);
	c.innerHTML = self.icons[a];
	self.etray.find('.pm_main_one').append(c);
};
pexxmoji.prototype.utils_rand = function(a){ const self = this;
	let b = '';
...
pexxmoji.prototype.utils_gcur = function(){ const self = this;
	let a = self.input[0] ,b = 0, c, d;
	if (self.window.getSelection) {
		c = self.window.getSelection();
		if (c.rangeCount) {
			d = c.getRangeAt(0);
			if (d.commonAncestorContainer.parentNode == a) { b = d.endOffset; }
		}
	} else if (self.window.document.selection && self.window.document.selection.createRange) {
		d = self.window.document.selection.createRange();
		if (d.parentElement() == a) {
			let e = self.window.document.createElement('span');
...
pexxmoji.prototype.utils_emfc = function(){ const self = this;
	let a = '';
	let b = JSON.parse(self.window.localStorage.getItem('pm-fv-ls'));
	if (b) { a = b; }
	return a;
};
pexxmoji.prototype.utils_emid = function(a, b){ const self = this;
	let c = [];
...
pexxmoji.prototype.utils_srge = function(a, b){ const self = this;
	if (b === undefined) { b = a }
	if ('selectionStart' in self.input[0]) {
		self.input[0].selectionStart = a;
		self.input[0].selectionEnd = b;
	} else if(self.input[0].setSelectionRange) {
		self.input[0].setSelectionRange(a, b);
	} else if(self.input[0].createTextRange) {
		let d = self.input[0].createTextRange();
		d.collapse(true);
...
/*
 * The following block is a utility object that extends from the pexxmoji object. It is meant
...
function mojiparser$to_html(a, jq){
	let b = mojiparser$mp_regx(), c, d = [], e = [], m = [];
	while (c = b.exec(a)) {
		let f = "", l = "", g = c[0], h = g.length;
		try {
			for (let i = 0;i < h; i++) {
				let j = c[0].codePointAt(i).toString(16).substring(0,1).toUpperCase();
				if (j != "D") {
...
function mojiparser$to_text(a, jq){
	let b = [], c = "";
	a.find("i").each(function(i) { let d = jq(this).data("pm-co"); if (d) { b.push(d) } });
	if (b.length > 0) {
		for (let aa = 0; aa < b.length; aa++) {
			if (c == "") { c = a.html().replace(/(<i\b[^>]*>)[^<>]*(<\/i>)/i,b[aa]); }
			else { c = c.replace(/(<i\b[^>]*>)[^<>]*(<\/i>)/i,b[aa]); }
		}
	} else { c = a.html(); }
	return c
		.replace(/&#10;/g, '\n')
